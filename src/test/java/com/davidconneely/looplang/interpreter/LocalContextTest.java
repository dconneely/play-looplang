...
class LocalContextTest {
    ...
    class ContextCreation {
        ...
        void definedProgram() {
            ...
            Optional<InterpreterContext> ctx = globalContext.getProgramContext("MYPROG", List.of("A", "B"));
        ...
        void argCountMismatch() {
            globalContext.setVariable("A", 10);
            assertThrows(InterpreterException.class, () -> globalContext.getProgramContext("MYPROG", List.of("A")));
        }

        @Test
        @DisplayName("throws when arg variable is undefined")
        void undefinedArg() {
            assertThrows(InterpreterException.class, () -> globalContext.getProgramContext("MYPROG", List.of("UNDEFINED1", "UNDEFINED2")));
        }
    }
    ...
    class VariablePassing {
        ...
        void initialisesParams() {
            globalContext.setVariable("A", 10);
            globalContext.setVariable("B", 20);
            InterpreterContext localContext = globalContext.getProgramContext("MYPROG", List.of("A", "B")).orElseThrow();
    ...
    class ProgramAccess {
        ...
        void nestedProgramNotAllowed() {
            ...
            globalContext.setVariable("B", 0);
            InterpreterContext localContext = globalContext.getProgramContext("MYPROG", List.of("A", "B")).orElseThrow();
            assertThrows(InterpreterException.class, () -> localContext.setProgram("NEWPROG", List.of(), List.of()));
        }
    }
    ...
    class OrThrowVariants {
        ...
        void throwsForUndefinedProgram() {
            assertThrows(InterpreterException.class, () -> globalContext.getProgramContextOrThrow("UNDEFINED", List.of()));
        }
