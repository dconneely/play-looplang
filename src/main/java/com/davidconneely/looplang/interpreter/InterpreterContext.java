...
public sealed interface InterpreterContext permits GlobalContext, LocalContext {
  ...
  default Optional<InterpreterContext> getProgramContext(
      final String name, final List<String> args) {
  ...
  default InterpreterContext getProgramContextOrThrow(final String name, final List<String> args) {
    return getProgramContext(name, args)
        .orElseThrow(
            () -> new InterpreterException("program `" + name + "` has not been defined yet"));
  }

  default int getVariableOrThrow(final String name) {
    return getVariable(name)
        .orElseThrow(
            () -> new InterpreterException("variable `" + name + "` has not been defined yet"));
  }
}
